...
export default async function WalletPage() {
  ...
  // Calculate per-account earnings (X)
  const xAccountsList = profile?.x_accounts || [];
  const xAccountsEarnings = xAccountsList.map((acc: any) => {
    let earned = 0, pending = 0, approvedCount = 0, submittedCount = 0;
    acc.task_claims?.forEach((claim: any) => {
      const val = Number(claim.tasks?.payment_amount) || 0;
      const bonus = Number(claim.bonus_amount) || 0;
    ...
    const handle = acc.x_handle ? `@${acc.x_handle}` : 'X Account';
    return {
      id: acc.id, username: handle, status: acc.status,
      earned, pending, approvedCount, submittedCount,
      isActive: profile.active_x_account_id === acc.id
    };
  });

  // Calculate per-account earnings (Quora)
  const quoraAccountsList = profile?.quora_accounts || [];
  const quoraAccountsEarnings = quoraAccountsList.map((acc: any) => {
    let earned = 0, pending = 0, approvedCount = 0, submittedCount = 0;
    acc.task_claims?.forEach((claim: any) => {
      const val = Number(claim.tasks?.payment_amount) || 0;
      const bonus = Number(claim.bonus_amount) || 0;
    ...
    const handle = acc.quora_username ? `q/${acc.quora_username}` : 'Quora Account';
    return {
      id: acc.id, username: handle, status: acc.status,
      earned, pending, approvedCount, submittedCount,
      isActive: profile.active_quora_account_id === acc.id
    };
  });
  ...
  const activeXAccountData = xAccountsEarnings.find((a: any) => a.isActive) || xAccountsEarnings[0];
  const activeQuoraAccountData = quoraAccountsEarnings.find((a: any) => a.isActive) || quoraAccountsEarnings[0];
    ...
    <div className="dashboard-content-container" style={{ maxWidth: '1100px', margin: '0 auto' }}>
      ...
      <div className="wallet-grid">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
          ...
          <div style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-subtle)', borderRadius: '16px', padding: '24px' }}>
            <h3 style={{ fontSize: '18px', fontWeight: 700, color: 'var(--text-primary)', marginBottom: '16px' }}>X Profile Earnings Breakdowns</h3>
            {xAccountsEarnings.length === 0 ? (
              <p style={{ color: 'var(--text-muted)', fontSize: '14px' }}>No linked X profiles detected.</p>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                {xAccountsEarnings.map((acc: any) => (
                  <div key={acc.id} style={{ padding: '16px', borderRadius: '12px', background: 'var(--bg-card)', border: `1px solid ${acc.isActive ? '#ffffff' : 'var(--border-subtle)'}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
                    <div style={{ minWidth: '180px', flex: 1 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px', flexWrap: 'wrap' }}>
                        <span style={{ fontSize: '15px', fontWeight: 600, color: 'var(--text-primary)' }}>{acc.username}</span>
                        {acc.isActive && (
                          <span style={{ fontSize: '11px', background: 'rgba(255,255,255,0.1)', color: '#ffffff', padding: '2px 8px', borderRadius: '12px', fontWeight: 600, display: 'inline-flex', alignItems: 'center', gap: '4px', whiteSpace: 'nowrap' }}>
                            <UserCheck size={12} /> Active Profile
                          </span>
                        )}
                      </div>
          ...
          <div style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-subtle)', borderRadius: '16px', padding: '24px' }}>
            <h3 style={{ fontSize: '18px', fontWeight: 700, color: 'var(--text-primary)', marginBottom: '16px' }}>Quora Profile Earnings Breakdowns</h3>
            {quoraAccountsEarnings.length === 0 ? (
              <p style={{ color: 'var(--text-muted)', fontSize: '14px' }}>No linked Quora profiles detected.</p>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                {quoraAccountsEarnings.map((acc: any) => (
                  <div key={acc.id} style={{ padding: '16px', borderRadius: '12px', background: 'var(--bg-card)', border: `1px solid ${acc.isActive ? '#b92b27' : 'var(--border-subtle)'}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
                    <div style={{ minWidth: '180px', flex: 1 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px', flexWrap: 'wrap' }}>
                        <span style={{ fontSize: '15px', fontWeight: 600, color: 'var(--text-primary)' }}>{acc.username}</span>
                        {acc.isActive && (
                          <span style={{ fontSize: '11px', background: 'rgba(185,43,39,0.1)', color: '#b92b27', padding: '2px 8px', borderRadius: '12px', fontWeight: 600, display: 'inline-flex', alignItems: 'center', gap: '4px', whiteSpace: 'nowrap' }}>
                            <UserCheck size={12} /> Active Profile
                          </span>
                        )}
                      </div>
          ...
          {activeRedditAccountData && (
            <div style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-subtle)', borderRadius: '16px', padding: '24px', borderLeft: '4px solid var(--accent-blue)' }}>
              <h3 style={{ fontSize: '16px', fontWeight: 700, color: 'var(--text-primary)', marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                <ShieldCheck size={18} color="var(--accent-blue)" /> Active Reddit Session
              </h3>
          ...
          {activeXAccountData && (
            <div style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-subtle)', borderRadius: '16px', padding: '24px', borderLeft: '4px solid #ffffff' }}>
              <h3 style={{ fontSize: '16px', fontWeight: 700, color: 'var(--text-primary)', marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                <ShieldCheck size={18} color="#ffffff" /> Active X Session
              </h3>
              <p style={{ fontSize: '14px', color: 'var(--text-secondary)', lineHeight: 1.5, marginBottom: '16px' }}>
                You are currently earning on X with <strong>{activeXAccountData.username}</strong>. Submissions completed with this profile will credit its individual ledger.
              </p>
              <div style={{ display: 'flex', gap: '32px', flexWrap: 'wrap' }}>
                <div>
                  <span style={{ fontSize: '12px', color: 'var(--text-muted)' }}>Account Earning</span>
                  <p style={{ fontSize: '20px', fontWeight: 700, color: 'var(--text-primary)', marginTop: '4px' }}>${activeXAccountData.earned.toFixed(2)}</p>
                </div>
                <div>
                  <span style={{ fontSize: '12px', color: 'var(--text-muted)' }}>Pending Clearance</span>
                  <p style={{ fontSize: '20px', fontWeight: 700, color: '#eab308', marginTop: '4px' }}>${activeXAccountData.pending.toFixed(2)}</p>
                </div>
              </div>
            </div>
          )}

          {activeQuoraAccountData && (
            <div style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-subtle)', borderRadius: '16px', padding: '24px', borderLeft: '4px solid #b92b27' }}>
              <h3 style={{ fontSize: '16px', fontWeight: 700, color: 'var(--text-primary)', marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                <ShieldCheck size={18} color="#b92b27" /> Active Quora Session
              </h3>
              <p style={{ fontSize: '14px', color: 'var(--text-secondary)', lineHeight: 1.5, marginBottom: '16px' }}>
                You are currently earning on Quora with <strong>{activeQuoraAccountData.username}</strong>. Submissions completed with this profile will credit its individual ledger.
              </p>
              <div style={{ display: 'flex', gap: '32px', flexWrap: 'wrap' }}>
                <div>
                  <span style={{ fontSize: '12px', color: 'var(--text-muted)' }}>Account Earning</span>
                  <p style={{ fontSize: '20px', fontWeight: 700, color: 'var(--text-primary)', marginTop: '4px' }}>${activeQuoraAccountData.earned.toFixed(2)}</p>
                </div>
                <div>
                  <span style={{ fontSize: '12px', color: 'var(--text-muted)' }}>Pending Clearance</span>
                  <p style={{ fontSize: '20px', fontWeight: 700, color: '#eab308', marginTop: '4px' }}>${activeQuoraAccountData.pending.toFixed(2)}</p>
                </div>
              </div>
            </div>
          )}
