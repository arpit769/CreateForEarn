-- =========================================================
-- UPDATE TASK ACQUIRED TIME / DURATION TO 1 HOUR
-- =========================================================

-- 1. Update release_expired_claims_secure to use 1 hour interval
...
DECLARE
  ...
  v_claim RECORD;
BEGIN
  -- Find all tasks that have expired claims (> 1 hour)
  FOR v_claim IN 
    SELECT DISTINCT task_id 
    FROM public.task_claims 
    WHERE status = 'claimed' AND claimed_at < v_one_hour_ago
  LOOP
...
-- 2. Update claim_task_secure to use 1 hour interval
...
BEGIN
  ...
  -- 2. Release expired claims for this specific task (> 1 hour)
  UPDATE public.task_claims
  SET status = 'expired'
  ...
  -- 6. Same post limit check for upvote/like/comment/subscribe tasks
  ...
  ELSIF v_platform = 'youtube' THEN
      -- YouTube Cooldown: 1 task of the same type per rolling 20 hours per account
      DECLARE
        v_yt_type_count INT;
      BEGIN
        SELECT COUNT(*)::INT INTO v_yt_type_count
        FROM public.task_claims tc
        JOIN public.tasks t ON tc.task_id = t.id
        WHERE tc.youtube_account_id = p_youtube_account_id
          AND tc.status IN ('approved', 'submitted')
        ...
        IF v_yt_type_count >= 1 THEN
          RETURN QUERY SELECT FALSE, ('Account cooldown reached: max 1 ' || v_task_type || ' task per 20 hours on this YouTube account. Please wait for cooldown.')::TEXT;
          RETURN;
        END IF;
      END;
  END IF;

  -- 8. All checks passed: insert the claim
  IF v_platform = 'youtube' THEN
    INSERT INTO public.task_claims (task_id, user_id, youtube_account_id, status, claimed_at)
    VALUES (p_task_id, p_user_id, p_youtube_account_id, 'claimed', v_time_now);
  ELSE
    INSERT INTO public.task_claims (task_id, user_id, reddit_account_id, status, claimed_at)
    VALUES (p_task_id, p_user_id, p_reddit_account_id, 'claimed', v_time_now);
  END IF;
